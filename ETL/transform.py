...
def transform_development_data(
   ...
   Args:
    ...
    # Also, "more than half of the years" doesn't feel sensitive enough, but we can come back to this -- ALEX
        ...
        print(f'[TRANSFORM INFO] Filtering {country_code} WDI data to only include Indicators with data for 50% or more of selected years')
    ...
    # Makes floats integers for improved visualizations
        development_data_transformed.iloc[:, 2:] = development_data_transformed.iloc[:, 2:].astype(int)
        print(f'[TRANSFORM INFO] Converting floats to integers in {country_code} WDI data')
